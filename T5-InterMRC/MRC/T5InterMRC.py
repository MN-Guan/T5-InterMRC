...
import transformers
from T5Model import T5ForConditionalGeneration, T5ForMRC, T5ForEDMRC, T5ForSepMRC
...
def train(model, t5_tokenizer, cached_features_file, device, args):
    train_features, eva_features, train_examples, eva_examples = load_data(t5_tokenizer, cached_features_file, args)
...
def T5SubModel(shared, encoder, decoder, config):
    sub_model = T5ForConditionalGeneration(config)
    sub_model.shared.load_state_dict(shared.state_dict())
    sub_model.encoder.load_state_dict(encoder.state_dict())
    sub_model.decoder.load_state_dict(decoder.state_dict())
    return sub_model
...
def inference(model, t5_tokenizer, device, eva_iterator, eva_examples, eva_features, args):
    model.eval()
    ...
    if args.model_name != 'T5-Shared':
        re_model = T5SubModel(model.shared, model.encoder, model.re_decoder, model.config)
        qa_model = T5SubModel(model.shared, model.encoder, model.qa_decoder, model.config)
    else:
        re_model = T5SubModel(model.shared, model.encoder, model.decoder, model.config)
        qa_model = T5SubModel(model.shared, model.encoder, model.decoder, model.config)
    ...
    for iterator_index, batch in enumerate(epoch_iterator):
        ...
        if args.model_name != 'T5-Independent':
            qa_input_ids, qa_attention_mask = get_qa_inputs(re_outputs, batch[0], t5_tokenizer)
            ...
            qa_outputs = qa_model.generate(input_ids=batch[0],
                                           attention_mask=batch[1],
                                           # num_beams=num_beams,
                                           min_length=1,
                                           max_length=max_qa_output_length,
        ...
        for i in range(batch[0].shape[0]):
            ...
            re_pred = t5_tokenizer.decode(re_pred, skip_special_tokens=True)
            qa_pred = t5_tokenizer.decode(qa_pred, skip_special_tokens=True)
...
def get_tokenizer(model_level):
    try:
        tokenizer = T5Tokenizer.from_pretrained(model_level)
    except:
        print('Retry to get the tokenizer!')
        tokenizer = get_tokenizer(model_level)
    return tokenizer

def load_state_dict(t5_model, mrc_model):
    mrc_model.shared.load_state_dict(t5_model.shared.state_dict()) 
    mrc_model.encoder.load_state_dict(t5_model.encoder.state_dict()) 
    mrc_model.re_decoder.load_state_dict(t5_model.decoder.state_dict()) 
    mrc_model.qa_decoder.load_state_dict(t5_model.decoder.state_dict()) 
    return mrc_model
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_level', type=str, default='t5-base')
    ...
    t5_tokenizer = get_tokenizer(args.model_level)
    config = T5Config.from_pretrained(args.model_level)
    config.tokenizer = t5_tokenizer
    config.f1_threshold = None
    config.left_threshold = args.left_threshold
    config.right_threshold = args.right_threshold
    if args.model_name == 'T5-Shared':
        model = T5ForEDMRC.from_pretrained(args.model_level)
        model.config = config
    elif args.model_name == 'T5-InterMRC':
        model = T5ForMRC(config)
        model = load_state_dict(t5_model, model)
    elif args.model_name == 'T5-Independent':
        model = T5ForSepMRC(config)
        model = load_state_dict(t5_model, model)
    else:
         raise ValueError("model_name must be one of the ['T5-Shared', 'T5-InterMRC', 'T5-Independent']!")
    assert args.batch_size % args.mini_batch_size == 0
    device = torch.device(args.device)
    model = model.to(device)
    with open(args.log_file_name, 'a', encoding='utf-8') as f:
        f.write(f'The MRC model has {count_parameters(model):,} parameters. \n')
    cached_features_file = os.path.join('./Data/', 'padding_{}_{}_{}.json'.format(args.mode, args.data_name, str(args.max_len)))
    train(model, t5_tokenizer, cached_features_file, device, args)
